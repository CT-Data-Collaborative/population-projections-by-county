...
##################################################################
#
# Processing Script for Population Projections by County
...
county_pop_proj_file <- dir(path_to_raw_data, recursive=T, pattern = "county") 
state_pop_proj_file <- dir(path_to_raw_data, recursive=T, pattern = "state")

county_pop_proj_df <- read.csv(paste0(path_to_raw_data, "/", county_pop_proj_file), stringsAsFactors = FALSE, header=T)
...
dont_include1 <- c("County", "2020", "2025", "Percent", "Total")
...
dont_include2 <- c("County", "2015", "2025", "Percent", "Total")
...
dont_include3 <- c("County", "2015", "2020", "Percent", "Total")
...
#Calculate each county as % of state total
...
for (i in 1:length(total_cols)) {
  ...
  total_val <- as.numeric(total_pop_proj[, get(total_cols[i])][which(total_pop_proj$County == "Connecticut")])
  total_pop_proj[, col_label] <- NA
...
for (i in 1:length(total_15_age_cols)) {
  ...
  #total_val <- as.numeric(total_pop_proj[, which_total_col])[which(total_pop_proj$County == "Connecticut")]
  total_pop_proj[, col_label] <- NA
...
total_pop_proj_long_fips <- merge(total_pop_proj_long, fips, by = "County", all=T)
...
total_pop_proj_long_fips <- total_pop_proj_long_fips %>% 
  select(County, FIPS, Year, Gender, Age.Cohort, Variable, Measure.Type, Value) %>% 
  arrange(County, Year, Gender, Age.Cohort, Measure.Type) %>% 
